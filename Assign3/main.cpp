...
// Shaders' filenames.
const char vertShaderFile[] = "assign3.vert";
const char fragShaderFile[] = "assign3.frag";

GLSLProgram shaderProg;  // Contains the shader program object.


// Light info. Must be a point light.
const glm::vec3 lightPosition = glm::vec3(-10.0f, 5.0f, 3.0f ); // Given in world space.
const glm::vec3 lightLookAt   = glm::vec3( 0.0f,  1.5f, 0.0f ); // Given in world space.
const glm::vec3 lightUpVector = glm::vec3( 0.0f, 1.0f, 0.0f );  // Given in world space.
...
static void SetUpShadowMapAndFBO(void)
{
	...
	glActiveTexture(texUnit); // Bind shadow map to texture unit 0
	GLuint depthTex;
	glGenTextures(1, &depthTex);
	glBindTexture(GL_TEXTURE_2D, depthTex);
	glTexStorage2D(GL_TEXTURE_2D, 1, texInternalFormat,
...
static void RenderShadowMap(void)
{
	...
	glPolygonOffset(3.0f, 2.0f);
	RenderObjects(viewMat,projMat);
	glm::mat4 B = glm:: mat4(vec4(0.5,0.0,0.0,0.0),vec4(0.0,0.5,0.0,0.0),vec4(0.0,0.0,0.5,0.0),vec4(0.5,0.5,0.5,1));
